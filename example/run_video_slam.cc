...
void mono_tracking(const std::shared_ptr<openvslam::config>& cfg,
...
#ifdef USE_PANGOLIN_VIEWER
    pangolin_viewer::viewer viewer(cfg, &SLAM, SLAM.get_frame_publisher(), SLAM.get_map_publisher(), SLAM.get_depthmap_publisher());
#elif USE_SOCKET_PUBLISHER
    ...
    float frame_time = float(frame_skip) / cfg->camera_->fps_;
    ...
    std::thread thread([&]() {
        while (is_not_end) {
            ...
            if (false) { //
                const auto wait_time = frame_time - track_time; //1.0 / cfg->camera_->fps_ - track_time;
                if (0.0 < wait_time) {
                    std::this_thread::sleep_for(std::chrono::microseconds(static_cast<unsigned int>(wait_time * 1e6)));
                }
            }

            for (int fidx=0; fidx<frame_skip-1 && is_not_end; fidx++) {
                bool grabbed = false;
                grabbed = video.grab();
            }
