...
system::system(const std::shared_ptr<config>& cfg, const std::string& vocab_file_path)
    ...
    depthmap_db_ = new data::map_database();
    ...
    depthmap_publisher_ = std::shared_ptr<publish::depthmap_publisher>(new publish::depthmap_publisher(cfg_, depthmap_db_));

    // tracking module
    tracker_ = new tracking_module(cfg_, this, map_db_, bow_vocab_, bow_db_);
    // mapping module
    mapper_ = new mapping_module(cfg_, map_db_, camera_->setup_type_ == camera::setup_type_t::Monocular);
    ...
    //new depthmap module
    depthmap_compute_ = new depthmap_computation_module(cfg_,depthmap_db_);

    //new dense pointcloud module
    dense_pointcloud_module_ = new dense_pointcloud_module(cfg_, depthmap_db_);
    ...
    depthmap_compute_->set_mapping_module(mapper_);
    depthmap_compute_->set_dense_pointcloud_module(dense_pointcloud_module_);
    mapper_->set_depthmap_module(depthmap_compute_);
    global_optimizer_->set_depthmap_module(depthmap_compute_);
}
...
system::~system() {
    ...
    depthmap_thread_.reset(nullptr);
    delete depthmap_compute_;
    depthmap_compute_ = nullptr;

    dense_pointcloud_thread_.reset(nullptr);
    delete dense_pointcloud_module_;
    dense_pointcloud_module_ = nullptr;
...
void system::startup(const bool need_initialize) {
    ...
    depthmap_thread_ = std::unique_ptr<std::thread>(new std::thread(&openvslam::depthmap_computation_module::run, depthmap_compute_));
    dense_pointcloud_thread_ = std::unique_ptr<std::thread>(new std::thread(&openvslam::dense_pointcloud_module::run, dense_pointcloud_module_));
}
...
void system::shutdown() {
    ...
    depthmap_compute_->request_terminate();
    dense_pointcloud_module_->request_terminate();
    // wait until they stop
    while (!mapper_->is_terminated()
           || !global_optimizer_->is_terminated()
           || global_optimizer_->loop_BA_is_running()
           || !depthmap_compute_->is_terminated()
           || !dense_pointcloud_module_->is_terminated()) {
    ...
    depthmap_thread_->join();
    dense_pointcloud_thread_->join();
...
void system::save_map_database(const std::string& path) const {
    ...
    //go dense
    io::map_database_io map_db_io_d(cam_db_,depthmap_db_);
    map_db_io_d.save_message_pack_dense(path + "_dense");
    map_db_io_d.save_point_cloud_dense(path + "_dense.ply");
    resume_other_threads();
}

void system::print_map() {
    //pause_other_threads();
    io::map_database_io map_db_io(cam_db_, map_db_, bow_db_, bow_vocab_);
    map_db_io.print_message_pack();
    //resume_other_threads();
}
...
const std::shared_ptr<publish::depthmap_publisher> system::get_depthmap_publisher() const {
    return depthmap_publisher_;
}
...
void system::pause_other_threads() const {
    ...
    //pause new deptmap module
    //depthmap_thread_
    if (depthmap_compute_ && !depthmap_compute_->is_terminated()) {
        depthmap_compute_ ->request_pause();
        while (!depthmap_compute_->is_paused() && !depthmap_compute_->is_terminated()) {
            std::this_thread::sleep_for(std::chrono::microseconds(5000));
        }
    }

    //pause new pcl module
    if (dense_pointcloud_module_ && !dense_pointcloud_module_->is_terminated()) {
        dense_pointcloud_module_ ->request_pause();
        while (!dense_pointcloud_module_->is_paused() && !dense_pointcloud_module_->is_terminated()) {
            std::this_thread::sleep_for(std::chrono::microseconds(5000));
        }
    }
}
...
void system::resume_other_threads() const {
    ...
    // resume the depthmap module
    if (depthmap_compute_) {
        depthmap_compute_->resume();
    }
    // resume the dense pcl module
    if (dense_pointcloud_module_) {
        dense_pointcloud_module_->resume();
    }
}
