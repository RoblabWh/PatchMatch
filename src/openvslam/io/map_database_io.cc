...
map_database_io::map_database_io(data::camera_database* cam_db, data::map_database* map_db)
    : cam_db_(cam_db), map_db_(map_db) {}

void map_database_io::save_message_pack_dense(const std::string& path) {
    ...
    bool bDense = true;
    map_db_->to_json(keyfrms, landmarks, bDense);
    nlohmann::json json{{"cameras", cameras}, {"landmarks", landmarks}, {"keyframes", keyfrms}};
    std::ofstream ofs(path, std::ios::out | std::ios::binary);

    if (ofs.is_open()) {
        spdlog::info("save the Dense PCL file of database to {}", path);
...
void map_database_io::save_point_cloud_dense(const std::string& path) {
    std::lock_guard<std::mutex> lock(data::map_database::mtx_database_);
    ...
    if (ofs.is_open()) {
        ...
        ofs << "ply\n"
               "format ascii 1.0\n"
               "element vertex " << map_db_->get_num_landmarks() << '\n';
        ...
        for (const auto lm : landmarks) {
            ...
            const Vec3_t &color = lm->getColor();
...
void map_database_io::print_message_pack() {
    std::lock_guard<std::mutex> lock(data::map_database::mtx_database_);
