...
public:
    ...
    //! Get the depthmap publisher
    const std::shared_ptr<publish::depthmap_publisher> get_depthmap_publisher() const;

    //! Get the frame publisher
    const std::shared_ptr<publish::frame_publisher> get_frame_publisher() const;

    //! print current keyframes
    void print_map();
...
private:
    ...
    //! depthmap computation module
    depthmap_computation_module* depthmap_compute_= nullptr;
    //! depthmap computation thread
    std::unique_ptr<std::thread> depthmap_thread_ = nullptr;

    //! dense pcl module
    dense_pointcloud_module* dense_pointcloud_module_ = nullptr;
    //! dense pcl thread
    std::unique_ptr<std::thread> dense_pointcloud_thread_ = nullptr;
    ...
    //! depthmap publisher
    std::shared_ptr<publish::depthmap_publisher> depthmap_publisher_ = nullptr;
