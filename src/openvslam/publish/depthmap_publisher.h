#ifndef OPENVSLAM_PUBLISH_DEPTHMAP_PUBLISHER_H
#define OPENVSLAM_PUBLISH_DEPTHMAP_PUBLISHER_H
...
public:
     ...
     * @param depthmap_db
     */
    depthmap_publisher(const std::shared_ptr<config>& cfg, data::map_database* depthmap_db);

    /**
     * Destructor
     */
    virtual ~depthmap_publisher();
     ...
     * @return number of landmarks in map
     */
    unsigned int get_landmarks_dense(std::vector<data::landmark*>& all_landmarks);

private:
    //! config
    std::shared_ptr<config> cfg_;
    //! map database
    data::map_database* depthmap_db_;
