...
static bool cmp_keyfm_ids(const data::keyframe* kfrm1, const data::keyframe* kfrm2) {
    ...
    int id1 = kfrm1->src_frm_id_; //src_frm_id_;
    int id2 = kfrm2->src_frm_id_; //src_frm_id_;
...
mapping_module::mapping_module(const std::shared_ptr<config>& cfg, data::map_database* map_db, const bool is_monocular)
    : cfg_(cfg), local_map_cleaner_(new module::local_map_cleaner(is_monocular)), map_db_(map_db),
...
void mapping_module::set_depthmap_module(depthmap_computation_module* depthmap_compute) {
    depthmap_compute_ = depthmap_compute;
}
...
void mapping_module::run() {
    ...
    while (true) {
        ...
        //queuing
        if (cur_keyfrm_->id_ > 1) {
            depthmap_compute_->queue_keyframe(cur_keyfrm_);
        }
        
        
        /*
        std::vector<data::keyframe*> all_keyfrms;
        std::vector<data::keyframe*> keyfrms_to_densify;
        if (cur_keyfrm_->id_ >= 30*(keyfrm_count+1)) { //
            all_keyfrms = map_db_->get_all_keyframes();
            keyfrm_count++;
            for (std::vector<data::keyframe*>::iterator it = all_keyfrms.begin(); it < all_keyfrms.end(); it++) {
                data::keyframe* keyfrm_ = *it;
                if (keyfrm_) {
                    if (!keyfrm_->will_be_erased() && keyfrm_->id_ >= 30*(keyfrm_count-1)) { //6*(keyfrm_count-1) dann: !keyfrm_->will_be_erased() && keyfrm_->id_ >= cur_keyfrm_->id_ - 6 && keyfrm_->id_ < cur_keyfrm_->id_ - 3
                        keyfrms_to_densify.push_back(keyfrm_);
                        
                    }
                }
            }
            std::sort(keyfrms_to_densify.begin(), keyfrms_to_densify.end(), cmp_keyfm_ids);
            depthmap_compute_->queue_keyframes(keyfrms_to_densify);
...
void mapping_module::reset() {
    ...
    depthmap_compute_->request_reset();
