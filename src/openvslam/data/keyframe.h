...
public:
    ...
    keyframe(const keyframe& keyfrm);
     ...
     */
    nlohmann::json to_json(bool bDense=false);

    cv::Mat get_rgb_thumb();
    cv::Mat get_gray_thumb();
    cv::Mat get_mask_thumb();
    camera::base* get_camera();
    void set_cleaned_result(dense::DepthmapCleanerResult* result);
    dense::DepthmapCleanerResult* cleaned_result_ = nullptr;
    dense::DepthmapCleanerResult* get_cleaned_result();
    ...
    void add_landmark_dense(landmark* lm, const unsigned int idx);
    void add_keypt_dense(cv::Vec2i &point);
    ...
    /**
     * Compute 10th and 90th percentile of depth
     */
    void computePercentileDepth_10_90(double &perc_10, double &perc_90); 
    ...
    unsigned int num_keypts_dense_;
    ...
    std::vector<cv::Vec2i> keypts_dense_;
    //! bearing vectors
    ...
    const cv::Mat rgb_thumb_;
    const cv::Mat mask_thumb_;
    const cv::Mat gray_thumb_;
...
private:
    ...
    std::vector<landmark*> landmarks_dense_;
