...
frame::frame(const cv::Mat& img_gray, const double timestamp,
             ...
             camera::base* camera, const float depth_thr, const int width, const int height,
             const cv::Mat& mask)
    : id_(next_id_++), bow_vocab_(bow_vocab), extractor_(extractor), extractor_right_(nullptr),
      timestamp_(timestamp), camera_(camera), depth_thr_(depth_thr), width_(width), height_(height) {
    // Get ORB scale
    update_orb_info();
    ...
    cv::resize(mask.clone(), mask_thumb_, cv::Size(width_,height_));
    cv::resize(img_gray.clone(), gray_thumb_, cv::Size(width_,height_));
}
...
void frame::set_rgb_thumb(const cv::Mat& rgb) {
    cv::resize(rgb.clone(), rgb_thumb_, cv::Size(width_, height_));
}
cv::Mat frame::get_mask_thumb() {
    return mask_thumb_;
}

cv::Mat frame::get_rgb_thumb() {
    return rgb_thumb_;
}

cv::Mat frame::get_gray_thumb() {
    return gray_thumb_;
}
