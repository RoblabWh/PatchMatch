...
keyframe::keyframe(const frame& frm, map_database* map_db, bow_database* bow_db)
    : // meta information
      ...
      rgb_thumb_(frm.rgb_thumb_.clone()),
      gray_thumb_(frm.gray_thumb_.clone()),
      mask_thumb_(frm.mask_thumb_.clone()) {
    // set pose parameters (cam_pose_wc_, cam_center_) using frm.cam_pose_cw_
    set_cam_pose(frm.cam_pose_cw_);
}

keyframe::keyframe(const keyframe& keyfrm) : 
    id_(keyfrm.id_), src_frm_id_(keyfrm.src_frm_id_), timestamp_(keyfrm.timestamp_), camera_(keyfrm.camera_), depth_thr_(keyfrm.depth_thr_),
    num_keypts_(keyfrm.num_keypts_), keypts_(keyfrm.keypts_), undist_keypts_(keyfrm.undist_keypts_), bearings_(keyfrm.bearings_), 
    keypt_indices_in_cells_(assign_keypoints_to_grid(keyfrm.camera_, keyfrm.undist_keypts_)), 
    stereo_x_right_(keyfrm.stereo_x_right_), depths_(keyfrm.depths_), descriptors_(keyfrm.descriptors_.clone()), 
    graph_node_(std::unique_ptr<graph_node>(new graph_node(this, false))), 
    num_scale_levels_(keyfrm.num_scale_levels_), scale_factor_(keyfrm.scale_factor_), log_scale_factor_(std::log(keyfrm.scale_factor_)),
    scale_factors_(feature::orb_params::calc_scale_factors(keyfrm.num_scale_levels_, keyfrm.scale_factor_)), 
    level_sigma_sq_(feature::orb_params::calc_level_sigma_sq(keyfrm.num_scale_levels_, keyfrm.scale_factor_)), 
    inv_level_sigma_sq_(feature::orb_params::calc_inv_level_sigma_sq(keyfrm.num_scale_levels_, keyfrm.scale_factor_)),
    landmarks_(std::vector<landmark*>(keyfrm.num_keypts_, nullptr)), rgb_thumb_(keyfrm.rgb_thumb_.clone()),
    num_keypts_dense_(0), gray_thumb_(keyfrm.gray_thumb_.clone()), mask_thumb_(keyfrm.mask_thumb_.clone()) {

        set_cam_pose(keyfrm.cam_pose_cw_);
        std::set<landmark*> lms_other = keyfrm.get_valid_landmarks();
        int idx=0;
        for (std::set<landmark*>::iterator it = lms_other.begin(); it != lms_other.end(); it++) {
            landmark* lm = *it;
            landmark* lm_copy = new landmark(lm);
            add_landmark(lm_copy, idx);
            idx++;
        }
}
...
void keyframe::set_cleaned_result(dense::DepthmapCleanerResult* result) {
    cleaned_result_ = result;
}

dense::DepthmapCleanerResult* keyframe::get_cleaned_result() {
    return cleaned_result_;
}

cv::Mat keyframe::get_rgb_thumb() {
        return rgb_thumb_;
}

cv::Mat keyframe::get_gray_thumb() {
        return gray_thumb_;
}

cv::Mat keyframe::get_mask_thumb() {
        return mask_thumb_;
}

camera::base* keyframe::get_camera() {
    return camera_;
}

nlohmann::json keyframe::to_json(bool bDense) {
    std::size_t size = bDense ? landmarks_dense_.size() : landmarks_.size();
    std::vector<int> landmark_ids(size, -1);
    // extract landmark IDs
    if (bDense) {
        for (unsigned int i = 0; i < landmark_ids.size(); ++i) {
            if (landmarks_dense_.at(i) && !landmarks_dense_.at(i)->will_be_erased()) {
                landmark_ids.at(i) = landmarks_dense_.at(i)->id_;
            }
        }
    }
    else {
    ...
    nlohmann::json keypts_json = bDense ? convert_keypoints_dense_to_json(keypts_dense_) : convert_keypoints_to_json(keypts_);
...
void keyframe::add_landmark_dense(landmark* lm, const unsigned int idx) {
    landmarks_dense_.push_back(lm);
}

void keyframe::add_keypt_dense(cv::Vec2i &point) {
    keypts_dense_.push_back(point);
    num_keypts_dense_++;
}
...
void keyframe::computePercentileDepth_10_90(double &perc_10, double &perc_90) {
    ...
    const Vec3_t trans_cw(cam_pose_cw(0,3), cam_pose_cw(1,3),cam_pose_cw(2,3));
    ...
    for (const auto lm : landmarks) {
        ...
        const Vec3_t pos_w = lm->get_pos_in_world();
        const auto pos_c = rot_cw * pos_w + trans_cw;
        depths.push_back(pos_c.norm());
    }
    std::sort(depths.begin(), depths.end());
    perc_10 = depths[depths.size()/10];
