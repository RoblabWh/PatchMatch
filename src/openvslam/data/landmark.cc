...
landmark::landmark(landmark *other_lm)
    : id_(other_lm->id_), first_keyfrm_id_(other_lm->first_keyfrm_id_), pos_w_(other_lm->pos_w_), ref_keyfrm_(other_lm->ref_keyfrm_),
      num_observable_(other_lm->num_observable_), num_observed_(other_lm->num_observed_), map_db_(other_lm->map_db_) {}
...
void landmark::setColor(float x, float y, float z) {
    color_ = Vec3_t(x,y,z);
}

Vec3_t landmark::getColor() const {
    return color_;
}
