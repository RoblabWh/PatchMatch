...
bool map_database::pop_front_keyframe() {
    std::lock_guard<std::mutex> lock(mtx_map_access_);
    
    bool bSuccess=true;
    {
        //find oldest keyframe -> find keyframe with smallest id
        int front_keyfrm_id = INT32_MAX;
        for (const auto id_keyframe : keyframes_) {
            if (id_keyframe.first < front_keyfrm_id) {
                front_keyfrm_id = id_keyframe.first;
            }
        }

        //remove oldest 
        keyframes_.erase(front_keyfrm_id);
        
        //remove landmarks associated with the erased keyframe ATTENTION POINTS NOT FREED
        openvslam::data::landmark* lm;
        std::unordered_map<unsigned int, landmark*> landmarks_new;
        std::unordered_map<unsigned int, openvslam::data::landmark*>::iterator it = landmarks_.begin();
        while (it != landmarks_.end()) {
            
            lm = it->second;
            if (lm->get_ref_keyframe()->id_ != front_keyfrm_id) {
                landmarks_new[lm->id_] = lm;
            }
            it++;
        }

        landmarks_.clear();
        landmarks_ = landmarks_new;
        /*
        for (const auto id_landmark : landmarks_) {
            lm = id_landmark.second;
            if (lm->get_ref_keyframe()->id_ == front_keyfrm_id) {
                int count = landmarks_.erase(lm->id_);
                if (count) {
                    bSuccess=true;
                }
            }
...
void map_database::to_json(nlohmann::json& json_keyfrms, nlohmann::json& json_landmarks, bool bDense) {
    std::lock_guard<std::mutex> lock(mtx_map_access_);
    ...
    for (const auto id_keyfrm : keyframes_) {
        ...
        keyfrms[std::to_string(id)] = keyfrm->to_json(bDense);
    }
