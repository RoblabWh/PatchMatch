...
public:
    ...
    Vec3_t convert_keypoint_to_bearing(const cv::KeyPoint& undist_keypt, int scale_down=1) const override final {
        // "From Google Street View to 3D City Models (ICCVW 2009)"
        Vec3_t bearing;

        // convert to unit polar coordinates
        const double longitude = (undist_keypt.pt.x / cols_*scale_down - 0.5) * (2.0 * M_PI);
        const double latitude = -(undist_keypt.pt.y / rows_*scale_down - 0.5) * M_PI;
