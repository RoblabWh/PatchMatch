...
private:
     ...
     * @param gl_cam_pose_wc
     */
    void follow_camera_dense(const pangolin::OpenGlMatrix& gl_cam_pose_wc);
    ...
    /**
     * Get the current camera pose via the depthmap publisher
     * @return
     */
    pangolin::OpenGlMatrix get_current_cam_pose_dense();
    ...
    /**
     * Get and draw keyframes via the depthmap publisher
     */
    void draw_keyframes_dense();
    ...
    /**
     * Get and draw landmarks via the depthmap publisher
     */
    void draw_landmarks_dense();
    ...
    //! depthmap publisher
    const std::shared_ptr<openvslam::publish::depthmap_publisher> depthmap_publisher_;
    ...
    // menu dense panel
    std::unique_ptr<pangolin::Var<bool>> menu_follow_camera_dense_;
    std::unique_ptr<pangolin::Var<float>> menu_frm_size_dense_;
    std::unique_ptr<pangolin::Var<float>> menu_lm_size_dense_;
    std::unique_ptr<pangolin::Var<bool>> menu_terminate_dense_;
    std::unique_ptr<pangolin::Var<bool>> menu_reset_dense_;


    // camera renderer
    std::unique_ptr<pangolin::OpenGlRenderState> s_cam_;
    std::unique_ptr<pangolin::OpenGlRenderState> s_cam_dense_;
    ...
    const std::string densemap_viewer_name_{"PangolinViewer: Dense Map Viewer"};
